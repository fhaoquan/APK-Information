...
(
    ...
    [datetime_file_access_human_readable] => was 33 mins ago
    [datetime_file_creation_human_readable] => was 33 mins ago
    [datetime_file_modification_human_readable] => was 33 mins ago
    [name] => info.staticfree.android.twentyfourhour
    [version] => 0.4.2
    [vercode] => 8
    [sdk_minimum_version] => 7
    [sdk_target_version] => 17
    [application_meta_data] => Array
        (
        )
...
## -=- JSON encrypt the output (its intended for it..)
## -=- Store as JSON file in same folder, same name of the apk file (but ends with .json ...)
## -=- Dump (PHP) a list of the JSON files, handle presentation, list rendering, image loading on *Client-Side* (JavaScript).
## -=- Make a lovely index to all your APK backups<br/>
<small> \*hhha \*cough\* \* not a d0wn1oad-webs!te...</small>
